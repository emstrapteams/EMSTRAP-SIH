    ...
    try {
        ...
        const booking = await Booking.findById(req.params.id)
            .populate({
                ...
                model: BookingDriver,
                select:
                    "name mobile email vehicleNumber currentLocation driverStatus isOnTrip",
            });
...
export const acceptBooking = async (req, res) => {
    try {
        ...
        console.log("Room:", `request_${req.params.id}`);
        console.log("==================================");
        ...
        console.log("✅ ambulance_assigned event emitted");
...
export const arriveBooking = async (req, res) => {
    try {
        ...
        io.to(`request_${req.params.id}`).emit("booking_updated", {
...
export const getPrivateDriverHistory = async (req, res) => {
    ...
    try {
        ...
        const { getBookingUserModel } = await import(
            "../models/bookingUser.model.js"
        );

        const { getBookingDriverModel } = await import(
            "../models/bookingDriver.model.js"
        );

        const BookingUser =
            getBookingUserModel(bookingConnection);
